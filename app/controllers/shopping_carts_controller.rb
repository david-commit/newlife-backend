class ShoppingCartsController < ApplicationController
    ...
    def show
        render json: ShoppingCart.find(params[:id]), serializer: ShoppingCartDetailedSerializer, status: :ok
    end

    def index
        if(params[:user_id])
            user = User.find(params[:user_id])
            render json: user.orders, each_serializer: OrderShoppingCartSerializer, status: :ok
        else
